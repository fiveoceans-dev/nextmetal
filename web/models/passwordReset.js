...
const PasswordReset = {
  ...
  findByToken: async (resetToken) => {
    const res = await pool.query(
      'SELECT * FROM "nextmetal-notebook".password_resets WHERE reset_token = $1 AND expires_at > NOW()',
      [resetToken]
    );
    return res.rows[0];
  },
  ...
  create: async (userId, resetToken) => {
    ...
    const result = await pool.query(
      'INSERT INTO "nextmetal-notebook".password_resets (user_id, reset_token, expires_at) VALUES ($1, $2, $3) RETURNING id',
